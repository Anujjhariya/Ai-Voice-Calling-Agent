...
from dynamic_ivr import router as ivr_router
from dynamic_script_manager import router as scripts_router
from web_agent import router as web_agent_router
from agent_outbound import router as agent_router
...
app = FastAPI(title="Binjwa IT Solutions — AI IVR + Web Agent")

# Allow the React frontend to communicate with this server
app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=r"https?://.*", # Allow all HTTP/HTTPS origins dynamically
...
# Serve static files (web_agent.html, etc.)
...
async def preload_greeting():
    ...
    print("⏳ Pre-generating greeting audio with HAPPY emotion...")
    try:
        GREETING_AUDIO = await generate_pcm(GREETING_TEXT, "en_happy")
        print(f"✅ Greeting cached: {len(GREETING_AUDIO)} bytes (HAPPY tone)")
...
def push_to_crm(payload: dict):
    ...
    webhook_url = os.getenv("IVR_CRM_WEBHOOK_URL")
...
async def trigger_twilio_call(request: Request):
    ...
    base_url = os.getenv("IVR_BASE_URL")
    account_sid = os.getenv("IVR_TWILIO_ACCOUNT_SID")
    auth_token = os.getenv("IVR_TWILIO_AUTH_TOKEN")
    twilio_phone = os.getenv("IVR_TWILIO_PHONE_NUMBER")
...
async def health_check():
    """Simple server health check API"""
