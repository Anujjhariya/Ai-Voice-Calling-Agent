...
async def pregenerate_web_greetings():
    ...
    print("[WebAgent] Pre-generating Web Agent greetings...")
    for lang, text in GREETING_TEXTS.items():
        if lang in greeting_audio_cache:
            continue
        try:
            audio_bytes = await generate_wav(text, lang_code=lang, gender="female")
            if audio_bytes:
                greeting_audio_cache[lang] = base64.b64encode(audio_bytes).decode("utf-8")
                print(f"[WebAgent] Cached web greeting ({lang}): {len(audio_bytes)} bytes")
