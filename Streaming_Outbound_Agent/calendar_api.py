...
CREDENTIALS_FILE = os.getenv("IVR_GOOGLE_CREDENTIALS_FILE", "credentials.json")
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                # Stale or revoked refresh token. Remove and reauthorize.
...
def book_meeting(name: str, dt_str: str) -> str:
    """
    Book a 30-minute meeting on Google Calendar if slot is free.
    - name: customer's name
    - dt_str: datetime string in format "YYYY-MM-DD HH:MM"
    Returns a confirmation message string or "ERROR_ALREADY_BOOKED".
    """
    try:
        ...
        # Check calendar availability for this 30-minute block
        ...
        if existing_events:
            print(f"⚠️ Conflict found: {len(existing_events)} overlapping events at {dt_str}")
