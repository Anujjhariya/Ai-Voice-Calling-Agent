...
def push_to_crm(payload: dict):
    """Push event data to the CRM Webhook in the background (non-blocking, 3s timeout)."""
...
class CampaignQueue:
    ...
    async def dial_next(self):
        ...
        try:
            await asyncio.to_thread(make_vobiz_ivr_call, phone, name)
            self.is_dialing = False
...
async def trigger_test_call(request: Request):
    ...
    vobiz_phone = os.getenv("VOBIZ_PHONE_NUMBER", "+911171366938")
...
async def trigger_single_call(request: Request):
    ...
    payload = {
        "from": vobiz_phone,
        "to": phone,
        "answer_url": f"{base_url}/ivr/welcome?name={encoded_name}",
        "answer_method": "GET",
...
<Response>
    <GetDigits numDigits="1" timeout="10"
        ...
        {say(f"Hello {name}, first of all congratulations for your new business! I am calling from बिंजवा IT Solutions. We provide complete solutions for your business from legal consultancy, compliance management, digital marketing, to website and app development. Meeting schedule करने के लिए 1 दबाएँ। हमारी sales team से बात करने के लिए 2 दबाएँ।")}
    </GetDigits>
...
async def ivr_handle_menu(request: Request):
    ...
    if not ivr_phone:
        ivr_phone = normalize_phone(os.getenv("VOBIZ_PHONE_NUMBER", ""))
    ...
    <Record action="{base_url}/ivr/confirm/1?name={encoded_name}" method="POST" maxLength="8" timeout="1" playBeep="true" />
    <Redirect method="POST">{base_url}/ivr/fallback_time/1?name={encoded_name}</Redirect>
        ...
        return xml_resp(f"""<Response>
    {say("ठीक है! हमारी sales team जल्द ही आपसे संपर्क करेगी। Binjva IT Solutions को समय देने के लिए धन्यवाद। आपका दिन शुभ हो।")}
    <Hangup/>
</Response>""")
    else:
        return xml_resp(f"""<Response>
...
async def ivr_book(key: str, request: Request):
    ...
    return xml_resp(f"""<Response>
    {say(prompt)}
    <Record action="{base_url}/ivr/book_time/{key}" method="POST" maxLength="6" timeout="1" playBeep="true" />
...
async def ivr_book_time(key: str, request: Request):
    ...
    <Record action="{base_url}/ivr/confirm/{key}?name={encoded_name}" method="POST" maxLength="8" timeout="1" playBeep="true" />
...
async def ivr_fallback_time(key: str, request: Request):
    ...
    if retry >= 2:
        return xml_resp(f"""<Response>
    {say(f"कोई बात नहीं {name} जी। आप बाद में हमें call कर सकते हैं। Binjva IT Solutions को समय देने के लिए बहुत धन्यवाद। आपका दिन शुभ हो।")}
    <Hangup/>
</Response>""")
    ...
    <Record action="{base_url}/ivr/confirm/{key}?name={encoded_name}" method="POST" maxLength="8" timeout="1" playBeep="true" />
    <Redirect method="POST">{base_url}/ivr/fallback_time/{key}?name={encoded_name}&amp;retry={retry + 1}</Redirect>
</Response>""")
    ...
    async def finalize_booking():
        try:
            ...
            if result != "ERROR_ALREADY_BOOKED":
                # Build a structured IVR transcript
                transcript = (
                    f"IVR: Hello {name}, I am calling from Binjwa IT Solutions. "
                    f"Meeting schedule करने के लिए 1 दबाएँ।\n"
                    f"Customer: [Pressed 1 - Meeting Book]\n"
                    f"IVR: कृपया बताएँ कि आप किस दिन और कितने बजे मीटिंग करना चाहते हैं?\n"
                    f"Customer (Voice): {time_speech}\n"
                    f"IVR: हमने आपकी {friendly_time} की request ले ली है।"
                )
                push_to_crm({
...
<Response>
    ...
    {say("हमारी team जल्द ही calendar check करके आपको SMS भेज देगी। Binjva IT Solutions को समय देने के लिए धन्यवाद!")}
    <Hangup/>
</Response>""")
